...
Stability-Constrained Continual Learning}
\author{Kavya Bhand \quad Aadi Joshi \\
\small Department of Computer Engineering, Vishwakarma Institute of Technology, Pune 411037, India \\
\small \texttt{\{kavya.bhand0806,toaadijoshi\}@gmail.com}}
\date{}
...
concluding it reflects task structure rather than model geometry. We show
this is incomplete: because the dual-ascent optimizer resets its Lagrange
...
spans roughly $1$ to $90$ on the \emph{same} model and task, exceeding the
effect of the original $24$-fold architecture range. This
...
non-negligible between-architecture structure. Applying our own diagnostic
standard to our own zoo design, however, we trace most of that structure
to two further, independently-diagnosed data-quality issues: a
...
sample, the four purely curvature-based statistics we test (Hessian trace,
...
learning is actually practiced, shows FTR still gives the lowest
forgetting and highest accuracy of the four methods, with an order of
magnitude less seed-to-seed variance than the next best. Together, these
...
\caption{Five architecturally distinct model families are usually assumed
to each require their own stability budget. This paper's central finding is
that, under Lagrangian-dual constrained continual learning, the budget is
instead set almost entirely by a shared control mechanism, the dual-ascent
optimizer's own schedule, common to every architecture trained with it.}
...
\paragraph{Central Research Question.} We ask whether this curvature-dependent
...
structure alone? We additionally ask a question the constrained-continual-learning
literature has not, to our knowledge, posed: \emph{is $\epsstar$ in fact
...
\paragraph{Primary Findings.} Contrary to the reading that stability
crossovers reflect task or architecture geometry alone, we find that the
...
(Section~\ref{sec:diagnostic}), more than the $24$--$75$-fold range in
architecture size moves it, and more than any single robustness axis we
test afterward (KL direction, task ordering, class-incremental vs.\
...
obscures (Section~\ref{sec:universality}); notably, applying our own
optimizer-schedule diagnostic to our own zoo design caught a second,
...
\begin{enumerate}
    \item We identify and directly test a confound in Lagrangian-dual
    constrained continual learning that, to our knowledge, no prior work on
    this optimization paradigm has isolated: because the dual variable is
    ...
    \item Holding the schedule fixed, we expand the architecture zoo from 8
    to 30 models across five representational families (CNN, ResNet, plain
    MLP, ViT, MLP-Mixer) and show, via a hierarchical partial-pooling model,
    ...
    that conclusion (Section~\ref{sec:universality}). We further show that
    part of the apparent family-level structure we initially observed was
    itself a second, self-inflicted instance of the schedule confound in our
    own zoo design, caught and corrected by applying our own diagnostic
    standard (Section~\ref{sec:selfconfound}), and that LwF's own
    ...
    \item We correct a methodological error in the original F-test for
    constancy (its code computed $p$ using $df_2=1000$ while the manuscript
    reported $df=(7,7)$) with a hierarchical partial-pooling model, and show
    ...
    \item We identify and test the specific ratio,
    $S=\lambda_{\mathrm{init}}/(\eta_\lambda N)$, that governs the
    schedule's effect on $\epsstar$, and show that holding it fixed while
...
matrix, thereby protecting parameters deemed important for prior tasks.
Synaptic Intelligence (SI) \citep{zenke2017continual} accumulates importance
weights online through path integrals over training trajectories. Both
methods enforce stability in parameter space, implicitly assuming that
maintaining proximity in parameter space ensures proximity in function space.
This assumption fails in overparameterized neural networks, where distinct
parameter vectors can produce functionally identical mappings
\citep{garipov2018loss,frankle2020linear,frankle2018lottery}. Recent work on
mode connectivity \citep{garipov2018loss,draxler2018essentially} has
established the non-convexity of the loss landscape in parameter space,
further motivating function-space approaches.
...
distributions through a KL-divergence distillation loss, maintaining the
student model's outputs close to the teacher model's predictions. Dark
Experience Replay (DER) \citep{buzzega2020dark} extends this by combining
distillation with stored logits. \citet{titsias2020functional} regularize
continual learning directly in function space via a KL term derived from a
Gaussian-process view of the network's outputs, and
\citet{pan2020continual} (FROMP) regularize network outputs at a set of
``memorable past'' points using a GP-derived functional prior with an
explicit memory. Both use a \emph{soft}, memory-based functional prior; FTR
differs in enforcing a \emph{hard} constraint via Lagrangian dual ascent with
no stored exemplars, and, unlike either, is evaluated here specifically for
whether its critical budget is architecture-independent.
...
paradigm and prove near-PAC learnability together with empirical duality-gap
bounds for non-convex losses, the theoretical setting that FTR's
Lagrangian dual-ascent update (Eqs.~\ref{eq:lagrangian}--\ref{eq:dual_ascent})
implicitly relies on. \citet{elenter2024nearoptimal} give near-optimal
solutions for this class of constrained-learning problems, and
\citet{hounie2023resilient} study \emph{resilient} constrained learning,
where the constraint level itself is relaxed adaptively based on cost,
directly relevant to how $\eps$ is chosen here and a natural extension we do
not pursue. Closest to the present work, \citet{elenter2023primal} formulate
continual learning explicitly as a Lagrangian-dual constrained-optimization
problem (Primal-Dual Continual Learning, PDCL) with a dual variable that
adapts via projected ascent on constraint violation, evaluated on
Tiny-ImageNet, SpeechCommands, and MedMNIST-OrganA in the class-incremental
setting. PDCL already demonstrates that Lagrangian-dual constrained continual
learning is practical and studies its dual dynamics; it does not, however,
constrain drift in \emph{function space} via a KL ball on output
distributions, and does not study whether its own critical constraint level
is architecture-independent, the two questions this paper is organized
around.

\subsection{Trust Regions and KL Constraints}
Trust Region Policy Optimization (TRPO) \citep{schulman2015trust} constrains
policy updates in reinforcement learning through explicit KL-divergence trust
regions, motivating the FTR formulation. The connection between KL
constraints and projected gradient descent in Riemannian function space has
been explored in online convex optimization \citep{hazan2019introduction,
rakhlin2012online,shalevshwartz2012online}. Recent work on natural gradient
methods \citep{amari1998natural,martens2015optimizing} establishes
connections between KL constraints and information geometry on parameter
manifolds.

\subsection{Curvature and Forgetting: an Active, More Recent Literature}
Our claim that curvature fails to predict $\epsstar$ under a function-space
hard constraint should not be read against the older curvature literature
alone \citep{keskar2016large,ghorbani2019investigation,li2018visualizing}; a
more recent, active line of work directly studies curvature-forgetting links
that our result must be positioned against rather than ignore.
\citet{cflat2024} use sharpness-aware minimization to reduce forgetting by
flattening the loss landscape, and its extension pushes this further; other
work explores Hessian eigenvalue spectra as a forgetting diagnostic, and
\citet{deng2021flattening} project gradients using curvature information to
reduce interference. Most directly relevant, \citet{parameterisolation2023}
formally derive that sharper old-task minima require a \emph{tighter}
parameter-space constraint to prevent forgetting. That result is not in
tension with the present finding: it is a parameter-space claim, and our
finding that curvature fails to predict $\epsstar$ is specific to
\emph{function}-space hard constraints. \citet{lewandowski2025spectral} and
\citet{lewandowski2024directions} study curvature's relationship to loss of
plasticity (the stability-plasticity tradeoff's other side), a complementary
rather than contradictory line.

\subsection{Phase Transitions in Deep Learning}
Phase transitions in neural network training have been documented in
contexts including grokking \citep{power2022grokking}, lottery-ticket
emergence \citep{frankle2018lottery}, and loss-landscape structure
\citep{li2018visualizing,keskar2016large}. Recent finite-size-scaling
treatments of grokking \citep{grokking2026dimensional} and of absorbing
phase transitions in deep networks \citep{scaling2023universal} apply
statistical-mechanics tools, tracking how a transition sharpens with system
size, directly analogous to the width-family analysis in
Section~\ref{sec:finitesize}. A thematically adjacent but mechanistically
different framing casts the stability-plasticity tradeoff itself as a
Stefan (moving-boundary) problem governed by a single scalar
\citep{stefancl2026}; we note it here as a kindred "physics-of-training"
perspective rather than a directly competing method.
...
We formulate task $t+1$ learning as a constrained optimization problem:
\begin{equation}
...
We solve the constrained problem using Lagrangian relaxation. The Lagrangian
is:
\begin{equation}
...
\label{eq:lagrangian}
\end{equation}
where $\lambda \ge 0$ is the dual variable. We employ dual gradient ascent
with momentum:
\begin{align}
...
v1's zoo comprised eight architectures, all convolutional or residual. We
expand it to 30 architectures spanning five representational families and a
...
we return to in Section~\ref{sec:selfconfound}). The forgetting metric is computed as the
mean decrease in accuracy on all previously learned tasks after completion of
training on the final task; we additionally report new-task accuracy
...
rescales the drift term itself). v1 varied only the first axis. We test the
second directly.

\subsection{Setup}
On two anchor architectures (CNN\_W16, CNN\_W32) and a 10-point $\eps$ grid
$\{1,3,5,6,7,8,9,10,12,20\}$, 3 seeds, we ran the \texttt{baseline}
...
$T \in \{1.0, 4.0\}$. For each of the 24 (architecture, condition) cells we
fit the same logistic-sigmoid procedure as Section~\ref{sec:crossover} and
bootstrap a 95\% CI (1{,}000 resamples over seeds). Four conditions produced
a forgetting curve that was \emph{flat across the entire $[1,20]$ grid}
(range $<0.05$) rather than an S-shape inside it, so we reran those four plus
...
located; we rescanned exactly those five (condition, architecture) cells
...
Three of the six hyperparameters we varied, $\eta_\lambda$,
...
continual learning rather than a CNN-specific quirk, we repeat the two
...
CNN-only version used for its own analogous cells). We do not draw
...
magnitude, comfortably exceeding the effect that a $24$--$75\times$ range in
architecture size produced in Section~\ref{sec:crossover}. So
...
method with a periodically reset dual variable. To our knowledge, no prior
...
has isolated the dual-ascent schedule itself as a confound for an apparent
stability threshold, and we are not aware of this question being asked
...
defaults (the same schedule used throughout v1), we ran the dense $17$-point
...
30-architecture zoo (full table in Appendix~\ref{app:fulltable}). Families: CNN
...
search-bound value diagnosed in Section~\ref{sec:diagnostic}; here the
...
trustworthy. We reran both on a wider grid extending to $\eps=200$ (5
...
determined than most of the zoo even with the wider grid. We use these
...
tracks two rounds of our own schedule confound rather than architecture
identity itself.
...
for both). \textbf{This clustering is substantially an artifact of our own
experimental design, not primarily a representational-paradigm effect}: as
...
than below it. We caught this only by applying Section~\ref{sec:diagnostic}'s
own standard to our own zoo, and report it plainly rather than let the
originally-recorded clustering stand as an architectural finding. One
...
methodology and the actual computation that we caught while reproducing it
(with $df_2=7$ the same statistic gives $p=0.767$, not $0.786$; the
discrepancy is not merely rounding). $df_2=1000$ has no stated
justification and appears to be an unexplained placeholder. We replace it
...
independently-diagnosed data-quality issues that we do not treat as
...
credible interval for $\tau$ still excludes zero, so we do not claim
...
"small" on standard interpretive scales ($<0.1$). We report both the raw
...
applied (from $r=-0.091$ and $-0.137$ on the uncorrected zoo). We flag this
...
Hessian/spectral estimators require by default; we force the math-mode
...
(Figure~\ref{fig:finitesize}). This is not statistically significant, and we
report it as a null result rather than force a narrative: at the widths
tested here, we find no reliable evidence that the FTR crossover sharpens
...
LwF/EWC comparison). We rerun FTR, LwF, EWC, and SI on the identical
...
pretrained backbone with a parameter-efficient adapter. We test this
...
Section~\ref{app:lorafix}'s finding that \texttt{nn.MultiheadAttention}'s
...
200 samples/class, 2 epochs/task, 3 seeds. We compare FTR ($\eps=5.0$,
...
this scale is not the point of this check.
...
asymmetric; we ablate the direction on CNN\_W16 and CNN\_W32 (dense $\eps$
...
task ID at evaluation. We additionally test the harder class-incremental
...
diagnostic obligates us to check for a schedule confound before reading it
...
pattern, comparable in magnitude to what we observe. We cannot, from this
...
We ran that corrected version: 10 architectures, classes/task
...
weakly positive rather than negative. We do not read this as evidence of a
...
we can fit reliably ($4$--$10$ classes/task), and the striking monotonic
...
suggests a way out: if this specific ratio, which we call $S$, is what
...
sub-zoo), we ran six conditions, all epoch/task
...
each pinned at $50e=54.37$ on this experiment's narrower grid); we reran
...
shows is possible when $S$ itself is left to vary. We do not claim this
...
substantial, practically useful reduction in dimensionality, and, to our
knowledge, the first time this specific ratio has been identified and
...
optimizer's own schedule. We now give an approximate, testable account of
...
self-consistently), so we treat it as a mechanistic account, not a closed-form
...
worth taking seriously, which brings us to a confound in our own zoo design.
...
the same $O(1/T)$ behavior that underlies our Eq.~\ref{eq:tstar}'s
...
step budget rather than by problem-specific constants alone; our diagnostic
sweep (Section~\ref{sec:diagnostic}) is, in effect, an empirical
...
constants it is usually assumed to be secondary to. We do not derive
...
to a task-transition hitting time, which we leave as a natural extension),
...
effects, is exactly what generic dual-ascent convergence theory predicts and
is not, to our knowledge, a connection the Lagrangian-dual continual-learning
literature has previously drawn out explicitly.
...
about our own experimental design. The ViT and MLP-Mixer families in
...
We tested this directly: re-running all four affected architectures
...
possibly almost entirely, an artifact of our own epoch/task mismatch, not
a genuine effect of representational paradigm (convolution vs.\ attention
vs.\ token-mixing). We report this plainly rather than let the earlier,
uncorrected framing stand, since it is exactly the kind of check
Section~\ref{sec:diagnostic} argues the field should be running as standard
practice, and we caught it only by holding ourselves to that standard
against our own zoo. Sections~\ref{sec:universality}.1 and
...
check sensitivity to these three quantities, not assume architecture is the
only relevant axis.
\item \textbf{Single dataset.} All main experiments use CIFAR-10; the
CIFAR-100 granularity sweep (Section~\ref{sec:taskstructure}) holds the
dataset fixed and varies only task construction. Tiny-ImageNet, CORe50, and
CLEAR-style benchmarks remain untested.
\item \textbf{Modest scale.} Moving to GPU compute (from v1's CPU-only
setup) enabled a $3.75\times$ larger architecture zoo and more seeds, but
model sizes ($\le2.8$M parameters) and per-task sample counts
($1{,}000$/class) remain small by contemporary standards.
\item \textbf{The theoretical account (Section~\ref{sec:theory}) is
approximate, not a closed-form derivation.} Equation~\ref{eq:eps_star_approx}
ignores dual-ascent momentum and treats the drift rate as constant in
$\lambda$; it matches the epochs/task sensitivity closely but under-predicts
the baseline value by an amount comparable to the schedule term itself, so
it should be read as a mechanistic account that generates testable
predictions (Section~\ref{sec:selfconfound}), not an exact formula.
\item \textbf{Aggregate and per-task-profile metrics only go so far.} We
report new-task accuracy and per-task forgetting profiles
(Section~\ref{sec:newtaskacc}), an improvement on v1's aggregate-only
reporting, but do not analyze per-class or per-sample forgetting patterns.
\item \textbf{No accuracy/SOTA comparison.} Unchanged from v1: this work is
diagnostic and mechanistic, not a proposal for a state-of-the-art
continual-learning method, and FTR without replay does not exceed LwF on
average accuracy.
\item \textbf{Curvature measurement required a backend workaround for one
family.} The Hessian trace / spectral norm estimators require a
twice-differentiable loss surface; PyTorch's fused scaled-dot-product-attention
kernel does not support this by default, which initially excluded the ViT
family from curvature measurement entirely. Forcing the math-mode SDPA
backend for curvature measurement only (Appendix~\ref{app:sdpa}) resolved
this, and the correlation results in Section~\ref{sec:universality} use the
full $n=30$ zoo.
\item \textbf{One architecture's crossover remains imprecisely determined
even after correction.} ResNetLite\_W8\_NoBN's wide-grid-relocated bootstrap
CI is $[6.87, 97.02]$ (Section~\ref{sec:crossover}); extending the search
grid resolved the exact bound-pinning artifact but not the underlying
measurement noise, which a purely $\eps$-grid-based fix cannot address. More
seeds, rather than a wider grid, are the likelier next step for this
specific architecture.
\item \textbf{The classes-per-task=2 condition is unreliable in the
corrected CIFAR-100 granularity experiment.} Holding samples/task fixed at
this granularity leaves few examples per class; fit quality there is
visibly worse than at classes/task $\ge4$ (Section~\ref{sec:granularityconfound}),
and we do not draw conclusions from that specific column.
\end{enumerate}

\section{Conclusion}
\label{sec:conclusion}

We set out to test a specific, previously unexamined confound in
...
a $75$-fold range in architecture size moves it, more than any other axis
we tested. A back-of-envelope mechanistic account
...
adequately powered one that does. What curvature does not predict, our own
zoo design initially seemed to: ViT and MLP-Mixer sat systematically
...
We report this correction plainly, because catching it is a demonstration
...
this family, ours included. Task ordering and the choice between
...
\citep{elenter2023primal,chamon2023constrained}. We see the diagnostic
...
\input{full_crossover_table_whole.tex}

\input{curvature_table_whole.tex}
...
ViT\_Small in our initial run. Wrapping only the curvature-measurement
...
silently bypassed and never receives a forward or backward pass. We
...
construction, training engine, Kaggle orchestration) are released with this
submission at \url{https://github.com/kavyabhand/Self-Stabilizing-ML-Inference}.
