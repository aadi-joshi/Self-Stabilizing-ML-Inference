...
from environment.random_degradation import RandomDegradation
...
# Environment selection: normal or unseen random degradation
env_type = config.get('environment_type', 'default')
if env_type == 'random':
    env = RandomDegradation(seed=random_seed)
else:
...
# Run all controllers under both default and unseen random degradation
from collections import deque
import pandas as pd
steps = 500
controller_results = {}
for env_mode in ['default', 'random']:
    if env_mode == 'random':
        env = RandomDegradation(seed=random_seed)
    else:
        env = EnvironmentDegradation(config['degradation'])
    for ctrl_name, controller in baseline_controllers.items():
        ...
        controller_results[f'{ctrl_name}_{env_mode}'] = logger.df.copy()
...
# Comparison plots for stability metrics by environment
import matplotlib.pyplot as plt
for metric in ['stability_horizon', 'oscillation_bound', 'recovery_time_mean']:
    plt.figure(figsize=(10, 6))
    for env_mode in stability_df['environment'].unique():
        subset = stability_df[stability_df['environment'] == env_mode]
        plt.bar(subset['controller'] + f' ({env_mode})', subset[metric], label=env_mode)
    plt.title(f'Stability Metric: {metric}')
    plt.ylabel(metric)
    plt.xticks(rotation=45)
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(plots_dir, f'stability_{metric}_comparison.png'))
    plt.close()
